...
from project_paths import (  # type: ignore # noqa: E402
    display_path,
...
LEGACY_PROFILE_YAML = "profile.yaml"
...
def normalize_profile_payload(payload: dict[str, Any], source: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if isinstance(data.get("stack"), str):
        data["stack"] = [item.strip().lower() for item in data["stack"].split(",") if item.strip()]
    ...
    if source.endswith(".yaml"):
        status = "legacy"
        message = "legacy YAML profile loaded; prefer project-state profile.json"
...
def load_project_profile(project_root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    profile_path = first_existing_project_state_path(project_root, PROFILE_JSON) or project_state_path(project_root, PROFILE_JSON)
    legacy_path = first_existing_project_state_path(project_root, LEGACY_PROFILE_YAML) or project_state_path(project_root, LEGACY_PROFILE_YAML)
...
def state_file_missing(project_root: Path, *parts: str) -> bool:
    return first_existing_project_state_path(project_root, *parts) is None


def collect_missing(project_root: Path, domains: list[str]) -> list[dict[str, str]]:
    missing: list[dict[str, str]] = []
    docs_root = first_existing_project_state_path(project_root, "project-docs")
    docs_root_display = display_path(project_root, project_state_path(project_root, "project-docs"))
    if docs_root is None and domains:
        missing.append(
            {
                "domain": "docs",
                "item": "role docs root",
                "path": docs_root_display,
                "severity": "warn",
    ...
    checks = {
        "frontend": [
            ("design system doc", ("project-docs", "frontend", "FE-02-design-system.md")),
            ("design tokens doc", ("project-docs", "frontend", "FE-03-design-tokens.md")),
            ("component inventory", ("project-docs", "frontend", "FE-04-component-inventory.md")),
        ],
        "backend": [
            ("API contracts doc", ("project-docs", "backend", "BE-01-api-contracts.md")),
            ("database design doc", ("project-docs", "backend", "BE-02-database-design.md")),
            ("auth/security doc", ("project-docs", "backend", "BE-04-auth-security.md")),
        ],
        "devops": [
            ("CI/CD doc", ("project-docs", "devops", "DO-02-ci-cd.md")),
            ("deployment runbook", ("project-docs", "devops", "DO-03-deployment-runbook.md")),
        ],
        "qa": [
            ("test strategy doc", ("project-docs", "qa", "QA-00-test-strategy.md")),
            ("regression map", ("project-docs", "qa", "QA-01-regression-map.md")),
        ],
    }
    for domain in domains:
        for item, parts in checks.get(domain, []):
            if state_file_missing(project_root, *parts):
                path = display_path(project_root, project_state_path(project_root, *parts))
...
def context_readiness(project_root: Path, profile_status: dict[str, Any]) -> dict[str, Any]:
    genome_path = first_existing_project_state_path(project_root, "context", "genome.md") or project_state_path(project_root, "context", "genome.md")
    knowledge_path = first_existing_project_state_path(project_root, "knowledge", "index.json") or project_state_path(project_root, "knowledge", "index.json")
    docs_index_path = first_existing_project_state_path(project_root, "project-docs", "index.json") or project_state_path(project_root, "project-docs", "index.json")
    specs_candidates = project_state_candidates(project_root, "specs")
    specs_root = next((path for path in specs_candidates if path.exists() and any(path.glob("*/SPEC.md"))), project_state_path(project_root, "specs"))
    return {
        "profile": profile_status,
        "genome": {"status": "present" if genome_path.exists() else "missing", "path": display_path(project_root, genome_path)},
        "role_docs": {"status": "present" if docs_index_path.exists() else "missing", "path": display_path(project_root, docs_index_path)},
        "knowledge": {"status": "present" if knowledge_path.exists() else "missing", "path": display_path(project_root, knowledge_path)},
        "specs": {
            "status": "present" if specs_root.exists() and any(specs_root.glob("*/SPEC.md")) else "missing",
            "path": display_path(project_root, specs_root),
        },
    }
