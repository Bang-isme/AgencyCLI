...
def read_codebase_index_summary(project_root: Path) -> dict[str, Any]:
    path = first_existing_project_state_path(project_root, "knowledge", "codebase-index.json") or project_state_path(
        project_root,
        "knowledge",
        "codebase-index.json",
    )
...
def main() -> int:
    ...
    file_count = args.file_count or defaults["file_count"]
    max_files = args.max_files or defaults["max_files"]
    budget_seconds = args.budget_seconds or defaults["budget_seconds"]
    require_graph = defaults["require_graph"]
