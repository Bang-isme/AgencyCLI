...
def load_recent_decisions(project_root: Path) -> List[str]:
    decisions_dir = first_existing_project_state_path(project_root, "decisions")
    if decisions_dir is None or not decisions_dir.exists() or not decisions_dir.is_dir():
        return []
    files = sorted(
...
def load_project_patterns(project_root: Path) -> List[str]:
    profile_path = first_existing_project_state_path(project_root, "project-profile.json")
    if profile_path is None or not profile_path.exists():
        return []
    try:
        profile = json.loads(safe_read(profile_path))
    except json.JSONDecodeError:
        return [f"- Unable to parse `{display_path(project_root, profile_path)}`."]
    ...
    if not bullets:
        bullets.append(f"- `{display_path(project_root, profile_path)}` exists but no recognized `patterns` entries were found.")
    return bullets


def load_current_issues(project_root: Path) -> List[str]:
    candidates: List[Path] = []
    for state_dir in project_state_candidates(project_root):
        if state_dir.exists():
            candidates.extend(path for path in state_dir.rglob("*.json") if path.is_file() and "tech" in path.name.lower())
...
def main() -> int:
    ...
    output_path = (
        ...
        if args.output
        else project_state_path(project_root, "handoff.md").resolve()
    )
