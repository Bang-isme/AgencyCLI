...
from project_paths import first_existing_project_state_path, project_state_path, user_skills_root  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  python generate_growth_report.py --project-root <project-root>\n"
            "  python generate_growth_report.py --project-root <project-root> --skills-root <skills-root> --since 30\n"
            "  python generate_growth_report.py --help\n\n"
            'Output:\n  JSON to stdout: {"status":"generated","path":"...","improvement_areas":N,...}'
        ),
    )
    parser.add_argument("--project-root", required=True, help="Project root path")
    parser.add_argument("--skills-root", default=str(user_skills_root()), help="Skills root path")
...
def ensure_output_path(project_root: Path, output_arg: str) -> Path:
    ...
    output_dir = project_state_path(project_root, "growth-reports").resolve()
...
def main() -> int:
    ...
    feedback_dir = first_existing_project_state_path(project_root, "feedback") or project_state_path(project_root, "feedback")
    sessions_dir = first_existing_project_state_path(project_root, "sessions") or project_state_path(project_root, "sessions")
