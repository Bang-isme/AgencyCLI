...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compact old project-state memory files to reduce context load.",
...
def compact_sessions(
    ...
    sessions_dir = project_state_dir(project_root) / "sessions"
...
def compact_feedback(project_root: Path, dry_run: bool) -> Tuple[int, int]:
    feedback_dir = project_state_dir(project_root) / "feedback"
...
def count_decisions(project_root: Path) -> int:
    decisions_dir = project_state_dir(project_root) / "decisions"
