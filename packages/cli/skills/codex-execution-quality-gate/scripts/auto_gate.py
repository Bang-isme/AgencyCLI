...
from project_paths import display_path, first_existing_project_state_path, project_state_path  # noqa: E402
...
def run_security(project_root: Path) -> RunnerResult:
    raw = cast(Dict[str, Any], security_scan.scan(project_root))
    critical = raw.get("critical")
    critical_count = len(critical) if isinstance(critical, list) else 0
    warnings_val = raw.get("warnings")
    warning_count = len(warnings_val) if isinstance(warnings_val, list) else 0
    payload = {
...
def run_pre_commit(project_root: Path) -> RunnerResult:
    raw_raw, exit_code = pre_commit_check.run_pre_commit(project_root, strict=False, skip_tests=False)
    raw = cast(Dict[str, Any], raw_raw)
    results_val = raw.get("results")
    results = results_val if isinstance(results_val, list) else []
    lint_status = summarize_result_statuses(
        [str(item.get("status", "skip")) for item in results if isinstance(item, dict) and str(item.get("check", "")) in {"eslint", "python_lint", "type_check"}]
    )
    test_status = summarize_result_statuses(
        [str(item.get("status", "skip")) for item in results if isinstance(item, dict) and str(item.get("check", "")) == "tests"]
    )
    warn_checks = [
        str(item.get("check", "unknown"))
        for item in results
        if isinstance(item, dict) and str(item.get("status", "")).strip().lower() == "warn"
    ]
    payload = {
...
def run_gate_check(project_root: Path) -> RunnerResult:
    raw = cast(Dict[str, Any], run_gate.build_gate_report(
    ...
    lint_status = summarize_gate_child(cast(Dict[str, Any], raw.get("lint", {}))) if isinstance(raw.get("lint"), dict) else "skip"
    test_status = summarize_gate_child(cast(Dict[str, Any], raw.get("test", {}))) if isinstance(raw.get("test"), dict) else "skip"
...
def run_tech_debt(project_root: Path) -> RunnerResult:
    raw = cast(Dict[str, Any], tech_debt_scan.scan_project(
        project_root=project_root,
...
def run_bundle(project_root: Path) -> RunnerResult:
    raw = cast(Dict[str, Any], bundle_check.analyze(project_root))
...
def run_specs(project_root: Path) -> RunnerResult:
    specs_root = first_existing_project_state_path(project_root, "specs")
    if specs_root is None or not specs_root.exists():
        payload = {
            "status": "skip",
            "overall": "skip",
            "matched_specs": 0,
            "unmapped_files": 0,
            "summary": "Spec advisory skipped because project-state specs are absent.",
        }
...
def run_knowledge(project_root: Path) -> RunnerResult:
    index_path = first_existing_project_state_path(project_root, "knowledge", "index.json")
    if index_path is None or not index_path.exists():
        payload = {
            "status": "skip",
            "overall": "skip",
            "summary": "Knowledge index advisory skipped because project-state knowledge/index.json is absent.",
        }
        return make_runner_result(payload)
    try:
    ...
    for parts in [("context", "genome.md"), ("project-docs", "index.json")]:
        source = first_existing_project_state_path(project_root, *parts) or project_state_path(project_root, *parts)
        if source.exists() and source.stat().st_mtime > index_mtime:
            stale_sources.append(display_path(project_root, source))
    payload = {
