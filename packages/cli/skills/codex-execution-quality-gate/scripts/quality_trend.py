...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="", help="Output directory (default: <project-state>/quality)")
...
def read_gate_events(output_dir: Path) -> List[Dict[str, Any]]:
...
def load_gate_events(output_dir: Path, days: int, anchor_date: Optional[date] = None) -> List[Dict[str, Any]]:
    cutoff = (anchor_date or date.today()) - timedelta(days=max(1, days) - 1)
    results: List[Dict[str, Any]] = []
    for payload in read_gate_events(output_dir):
        d = parse_event_timestamp(str(payload.get("date", "")))
        if d is not None and d >= cutoff:
            results.append(payload)
    return results


def summarize_gate_events(events: List[Dict[str, Any]]) -> Dict[str, Any]:
    if not events:
        return {
            "runs": 0,
            "gate_pass_rate": 0.0,
...
def load_snapshots(output_dir: Path, days: int, anchor_date: Optional[date] = None) -> List[Dict[str, Any]]:
    snapshots_dir = output_dir / "snapshots"
...
def compute_health_score(
    ...
    gate_quality: Optional[Dict[str, Any]] = None,
) -> int:
    score = 100
...
def build_report(output_dir: Path, days: int) -> Dict[str, object]:
    ...
    metrics_first = first_snapshot.get("metrics")
    metrics_first_dict = metrics_first if isinstance(metrics_first, dict) else {}
    first_metrics: Dict[str, float] = {}
    for k, v in metrics_first_dict.items():
        if isinstance(v, (int, float)):
            first_metrics[str(k)] = float(v)
 
    metrics_latest = latest_snapshot.get("metrics")
    metrics_latest_dict = metrics_latest if isinstance(metrics_latest, dict) else {}
    latest_metrics: Dict[str, float] = {}
    for k, v in metrics_latest_dict.items():
        if isinstance(v, (int, float)):
            latest_metrics[str(k)] = float(v)
...
def infer_overall_trend(trend_map: Dict[str, Any]) -> str:
    improving = 0
    declining = 0
    for key in ("todo_count", "long_functions", "long_files", "test_to_source_ratio"):
...
def emit_with_human(payload: Dict[str, Any], human: bool) -> None:
    emit(payload)
    if human:
...
def main() -> int:
    ...
    if args.output_dir:
        ...
        output_dir = project_state_path(project_root, "quality")
