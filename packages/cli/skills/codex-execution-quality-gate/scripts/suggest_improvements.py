...
def collect_large_file_suggestion(file_path: Path, rel_file: str) -> Optional[Dict[str, Any]]:
...
def collect_todo_suggestions(rel_file: str, lines: List[str], scope_lines: Set[int]) -> List[Dict[str, Any]]:
...
def collect_duplicate_suggestion(rel_file: str, lines: List[str]) -> Optional[Dict[str, Any]]:
    window = 3
    if len(lines) < window:
        return None
...
def collect_magic_number_suggestion(rel_file: str, lines: List[str], scope_lines: Set[int]) -> Optional[Dict[str, Any]]:
...
def collect_deep_nesting_suggestion(rel_file: str, lines: List[str], scope_lines: Set[int]) -> Optional[Dict[str, Any]]:
...
def collect_debug_suggestions(rel_file: str, lines: List[str], scope_lines: Set[int], extension: str) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
...
def format_summary(suggestions: List[Dict[str, Any]], scanned_count: int) -> str:
    if not suggestions:
