...
def scan_js_ts_functions(
    ...
    rel = rel_path(file_path, root)
    for block in extract_js_blocks(lines, rel, warnings):
        line_start = int(str(block.get("line_start", "0")))
        line_end = int(str(block.get("line_end", "0")))
...
def scan_unused_exports(
    ...
    deduped.sort(key=lambda item: (str(item.get("file", "")), int(item.get("line", 0)), str(item.get("name", ""))))
    return deduped


def build_summary(by_category: Dict[str, List[Dict[str, Any]]], total: int) -> str:
    return (
