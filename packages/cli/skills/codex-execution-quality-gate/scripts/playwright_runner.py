...
def emit(payload: Dict[str, object], exit_code: int = 0) -> NoReturn:
...
def collect_failures(report: Dict[str, Any]) -> List[Dict[str, str]]:
...
def parse_run_summary(report: Optional[Dict[str, Any]], fallback_exit: int, elapsed_ms: int) -> Dict[str, Any]:
    if report is None:
