...
def run_command(project_root: Path, spec: CommandSpec) -> Dict[str, Any]:
    try:
        proc = subprocess.run(
            spec.command,
            cwd=project_root,
        ...
        stderr_decoded = exc.stderr.decode("utf-8", errors="replace") if isinstance(exc.stderr, bytes) else (exc.stderr or "")
        stdout_decoded = exc.stdout.decode("utf-8", errors="replace") if isinstance(exc.stdout, bytes) else (exc.stdout or "")
        return {
            "ok": False,
            "exit_code": -1,
