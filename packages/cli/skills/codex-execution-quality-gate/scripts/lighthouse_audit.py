...
def to_score(value: Any) -> Optional[int]:
    if value is None:
        return None
    try:
...
def extract_opportunities(report: Dict[str, object], limit: int = 5) -> List[Dict[str, Any]]:
    """Extract Lighthouse opportunity audits."""
    audits = report.get("audits", {}) if isinstance(report, dict) else {}
    if not isinstance(audits, dict):
        return []
...
def main() -> None:
    ...
    output_dir = Path(args.output_dir).expanduser().resolve() if args.output_dir else project_state_path(Path.cwd(), "lighthouse")
