...
START_MARKER = "# >>> AGENCY QUALITY GATE START >>>"
END_MARKER = "# <<< AGENCY QUALITY GATE END <<<"
SKILLS_REPO_URL = "https://github.com/your-org/AgencyCLI.git"
...
def github_workflow() -> str:
    return f"""name: Agency Quality Gate
  ...
  gate:
    ...
    steps:
      ...
      - uses: actions/setup-python@v5
        with:
          python-version: '3.11'
      - name: Resolve Agency Skills
        shell: bash
        run: |
          if [ -d ".agency/skills" ]; then
            echo "AGENCY_SKILLS_ROOT=$PWD/.agency/skills" >> "$GITHUB_ENV"
          elif [ -d ".codex/skills" ]; then
            echo "AGENCY_SKILLS_ROOT=$PWD/.codex/skills" >> "$GITHUB_ENV"
          else
            git clone --depth 1 {SKILLS_REPO_URL} "$RUNNER_TEMP/agency-skills"
            echo "AGENCY_SKILLS_ROOT=$RUNNER_TEMP/agency-skills/skills" >> "$GITHUB_ENV"
          fi
      - name: Security Scan
        run: python "$AGENCY_SKILLS_ROOT/codex-execution-quality-gate/scripts/security_scan.py" --project-root .
      - name: Run Gate
        run: python "$AGENCY_SKILLS_ROOT/codex-execution-quality-gate/scripts/run_gate.py" --project-root .
"""
...
def write_github_workflow(output_path: Path, force: bool) -> None:
    if output_path.exists():
        ...
        if "CodexAI Quality Gate" not in existing_text and "Agency Quality Gate" not in existing_text and not force:
...
agency_quality_gate:
  ...
  before_script:
    - |
      if [ -d ".agency/skills" ]; then
        export AGENCY_SKILLS_ROOT="$PWD/.agency/skills"
      elif [ -d ".codex/skills" ]; then
        export AGENCY_SKILLS_ROOT="$PWD/.codex/skills"
      else
        git clone --depth 1 {SKILLS_REPO_URL} /tmp/agency-skills
        export AGENCY_SKILLS_ROOT="/tmp/agency-skills/skills"
      fi
  script:
    - python "$AGENCY_SKILLS_ROOT/codex-execution-quality-gate/scripts/security_scan.py" --project-root .
    - python "$AGENCY_SKILLS_ROOT/codex-execution-quality-gate/scripts/run_gate.py" --project-root .
  only:
    - branches
    - merge_requests
{END_MARKER}
