...
def execute_output_guard(
    ...
    try:
        if output_text is not None:
            ...
            path = resolve_optional_path(project_root, output_file)
...
def load_gate_state(project_root: Path) -> Dict[str, Any]:
    """Load persistent gate state from project-state state/gate_state.json."""
    state_file = first_existing_project_state_path(project_root, "state", "gate_state.json") or project_state_path(
        project_root,
        "state",
        "gate_state.json",
    )
...
def save_gate_state(project_root: Path, state: Dict[str, Any]) -> None:
    """Persist gate state to project-state state/gate_state.json."""
    state_file = project_state_path(project_root, "state", "gate_state.json")
    state_file.parent.mkdir(parents=True, exist_ok=True)
    state_file.write_text(json.dumps(state, indent=2), encoding="utf-8")


def append_gate_event(project_root: Path, report: Dict[str, Any]) -> None:
    quality_dir = project_state_path(project_root, "quality")
