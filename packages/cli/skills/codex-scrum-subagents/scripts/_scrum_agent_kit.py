...
from project_paths import project_state_path, user_state_path  # noqa: E402
...
STAMP_FILE = ".agencyai-scrum-kit.json"
LEGACY_STAMP_FILE = ".codexai-scrum-kit.json"
PROJECT_NATIVE_AGENTS_REL = Path(".agency") / "agents"
...
def project_codex_agents_root(target_root: Path) -> Path:
    return project_state_path(target_root, "agents")


def personal_codex_agents_root() -> Path:
    return user_state_path("agents")
...
def default_backup_root(target_root: Path) -> Path:
    ...
    return target_root / ".agencyai-backups" / f"scrum-agent-kit-{timestamp}"
...
def write_stamp(install_root: Path, target_root: Path, stats: BundleStats, force: bool, operation: str) -> Path:
    ...
    agents_root = project_codex_agents_root(resolved_target)
    try:
        agents_rel = relative_posix(agents_root, resolved_target)
    except ValueError:
        agents_rel = str(agents_root)
    stamp_payload = {
        ...
        "project_codex_agents_rel": agents_rel,
    }
...
def read_stamp(install_root: Path) -> Dict[str, object]:
    ...
    if not stamp_path.exists():
        stamp_path = install_root / LEGACY_STAMP_FILE
    if not stamp_path.exists():
        return {}
    try:
