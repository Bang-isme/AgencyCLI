...
def project_agents_label(target_root: Path, agents_root: Path) -> str:
    try:
        return relative_posix(agents_root.resolve(), target_root.resolve())
    except ValueError:
        return str(agents_root)


def personal_agents_label(agents_root: Path) -> str:
    return f"~/{agents_root.parent.name}/{agents_root.name}"
        ...
        if args.diff:
            ...
            if changed_paths or changed_project_native_paths or changed_personal_native_paths:
                ...
                backed_up_files += backup_existing_files(
                    ...
                    backup_root=backup_root / Path(project_agents_label(target_root, codex_agents_root)),
                ...
                if personal_agents_root is not None:
                    backed_up_files += backup_existing_files(
                        ...
                        backup_root=backup_root / "_personal" / Path(personal_agents_label(personal_agents_root).replace("~/", "")),
            ...
            project_prefix = project_agents_label(target_root, codex_agents_root)
            personal_prefix = personal_agents_label(personal_agents_root) if personal_agents_root is not None else "~/.agency/agents"
            all_conflicts = conflicts + [f"{project_prefix}/{name}" for name in project_native_conflicts]
            all_conflicts += [f"{personal_prefix}/{name}" for name in personal_native_conflicts]
