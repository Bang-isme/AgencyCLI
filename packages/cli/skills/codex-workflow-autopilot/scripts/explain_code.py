...
def load_shared_js_parser():
    ...
    spec = importlib.util.spec_from_file_location("_shared_js_parser", parser_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Cannot load JS parser spec: {parser_path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    spec.loader.exec_module(module)
    if not hasattr(module, "extract_js_blocks"):
        raise ImportError("extract_js_blocks not found in shared JS parser")
    return module.extract_js_blocks


try:
    extract_js_blocks = load_shared_js_parser()
except (ImportError, OSError):
    def extract_js_blocks(lines: List[str], rel_file: str, warnings: List[str]) -> List[Dict[str, Any]]:
        warnings.append(f"Shared JS parser unavailable for {rel_file}")
        return []
...
def parse_python_file(file_path: Path, root: Path, warnings: List[str]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
...
def parse_js_file(file_path: Path, root: Path, warnings: List[str]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    source = read_text(file_path)
    if not source:
        return [], []
    ...
    functions = extract_js_blocks(lines, rel, warnings)
    imports: List[Dict[str, Any]] = []
    ...
    unique_functions: Dict[Tuple[str, int, int], Dict[str, Any]] = {}
...
def dedupe_imports(imports: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def filter_functions(functions: List[Dict[str, Any]], focus_name: str) -> List[Dict[str, Any]]:
