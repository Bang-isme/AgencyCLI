...
from project_paths import project_state_dir  # noqa: E402
...
def render_spec(title: str, prompt: str, domains: list[str], state_dir_name: str = ".agency") -> str:
...
- [ ] TICKET-001: Establish requirements and evidence baseline.
  ...
  - Likely Files: `{state_dir_name}/specs/<slug>/SPEC.md`, project docs, tests, and files discovered by repo search.
  - Dependencies: None.
...
- [ ] TICKET-003: Verify, trace, and hand off.
  ...
  - Likely Files: `{state_dir_name}/specs/<slug>/SPEC.md`, test files, role docs, handoff notes if used.
...
- Run spec traceability check when `{state_dir_name}/specs/` exists.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create <project-state>/specs/<slug>/SPEC.md.")
...
def main() -> int:
    ...
    try:
        ...
        state_root = project_state_dir(project_root)
        spec_dir = state_root / "specs" / slug
        ...
        if spec_path.exists() and not args.force:
            ...
            spec_path.write_text(
                render_spec(args.title.strip(), args.prompt, parse_csv(args.domains), state_root.name),
                encoding="utf-8",
            )
