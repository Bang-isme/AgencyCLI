...
def project_harness_dir(project_root: Path) -> Path:
    agencyai = project_root / ".agencyai"
    codexai = project_root / ".codexai"
    if agencyai.exists() or not codexai.exists():
        return agencyai
    return codexai
...
def write_generic_manifest(project_root: Path, skills_target: Path, apply: bool) -> dict[str, Any]:
    harness_dir = project_harness_dir(project_root)
    harness_rel = harness_dir.name
    path = harness_dir / "harness.json"
    payload = {
        "schema_version": "1.0",
        ...
        "portable_entrypoints": {
            "route_prompt": f"python {harness_rel}/skills/.system/scripts/prompt_router.py --prompt <text> --format json",
            "trust_harness": f"python {harness_rel}/skills/.system/scripts/trust_harness.py --project-root . --setup generic --format json",
            "quality_gate": f"python {harness_rel}/skills/codex-execution-quality-gate/scripts/auto_gate.py --project-root . --mode quick",
        },
...
def write_pre_prompt_hook(project_root: Path, skills_target: Path, apply: bool) -> dict[str, Any]:
    path = project_harness_dir(project_root) / "hooks" / "pre_prompt.json"
...
def setup_generic(project_root: Path, skills_root: Path, apply: bool) -> dict[str, Any]:
    target = project_harness_dir(project_root) / "skills"
...
def run_tests(skills_root: Path, checks: list[dict[str, Any]], skip_tests: bool) -> None:
    ...
    smoke_path = skills_root / "tests" / "smoke_test.py"
    if not smoke_path.exists():
        add_check(
            checks,
            "smoke",
            "pass",
            "smoke_test.py not present (skipped)",
            skipped=True
        )
    else:
