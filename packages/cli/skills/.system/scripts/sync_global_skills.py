...
HELPER_DIR = Path(__file__).resolve().parents[1] / "lib"
if str(HELPER_DIR) not in sys.path:
    sys.path.insert(0, str(HELPER_DIR))

from project_paths import user_skills_root  # noqa: E402
...
def default_backup_dir(global_root: Path) -> Path:
    ...
    return global_root / ".agencyai-backups" / f"sync-{timestamp}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sync CodexAI skills to the global skills root.")
    parser.add_argument("--source-root", default="", help="Source skills root. Defaults to this script's skills root")
    parser.add_argument("--global-root", default="", help="Global skills root. Defaults to the shared user skills root")
