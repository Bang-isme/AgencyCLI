    ...
    if manifest:
        ...
        for skill_name in manifest.get("skills", []):
            ...
            if not skill_dir.is_dir():
                continue
            for sub in ("templates", "starters", "scripts", "references", "assets"):
                sub_dir = skill_dir / sub
                if sub_dir.is_dir():
                    files = [p for p in sub_dir.rglob("*") if p.is_file()]
                    if not files:
                        asset_failures.append(f"{skill_name}/{sub}: directory is empty")
                    for f in files:
                        if f.suffix == ".py":
                            try:
                                compile(f.read_text(encoding="utf-8-sig", errors="replace"), str(f), "exec")
                            except SyntaxError as e:
                                asset_failures.append(f"{skill_name}/{sub}/{f.name}: Python syntax error: {e}")

        add(
            checks,
            "source_skills_assets_completeness",
            "pass" if not asset_failures else "fail",
            "all skill templates, starters, and scripts are present and syntactically valid" if not asset_failures else "; ".join(asset_failures[:5]),
...
def check_global_sync(source_root: Path, global_root: Path) -> list[dict[str, Any]]:
    ...
    # Dynamic recursive comparison of source vs global to guarantee 100% complete injection of all assets (templates, starters, scripts)
    SKIP_NAMES = {
        ".analytics", ".agency", ".agencyai", ".agencyai-backups", ".codex", ".codexai",
        ".codexai-backups", ".coverage", ".git", ".mypy_cache", ".pytest_cache", ".ruff_cache",
        "__pycache__", "build", "cache", "dist", "htmlcov", "node_modules", "state"
    }
    ...
    for item in sorted(source_root.rglob("*"), key=lambda p: p.as_posix().lower()):
        if any(part in SKIP_NAMES for part in item.relative_to(source_root).parts):
            continue
        ...
        global_item = global_root / rel
        
        if not global_item.exists():
            sync_failures.append(f"{rel}: missing in global root")
        elif global_item.stat().st_size != item.stat().st_size:
            sync_failures.append(f"{rel}: size mismatch (source={item.stat().st_size}, global={global_item.stat().st_size})")
        else:
            synced_files_count += 1

    add(
        checks,
        "global_skills_assets_sync",
        "pass" if not sync_failures else "fail",
        f"100% sync verified: {synced_files_count}/{source_files_count} files match perfectly" if not sync_failures else f"Sync drift detected: {len(sync_failures)} file(s) mismatched/missing",
        failures=sync_failures[:10],
        total_source=source_files_count,
