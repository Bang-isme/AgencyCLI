...
def check_native_agents(agent_root: Path, checks: list[dict[str, Any]], check_name: str = "native_agents") -> None:
    if not agent_root.exists():
        add(checks, check_name, "pass", f"native agent root absent: {agent_root}")
        return
    try:
        import tomllib
    except ModuleNotFoundError:
        add(checks, check_name, "warn", "tomllib unavailable; skipped TOML validation")
        return
...
def validate(plugin_root: Path, strict: bool = False) -> dict[str, Any]:
    ...
    check_native_agents(root / ".agency" / "agents", checks, "native_agents_agency")
    check_native_agents(root / ".codex" / "agents", checks, "native_agents_codex")
    return summarize(checks, strict)
