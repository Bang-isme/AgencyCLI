#!/usr/bin/env python3
"""Initialize role-scoped project documentation under project state."""
...
def docs_root(project_root: Path) -> Path:
    return project_state_path(project_root, "project-docs")
...
def initialize_role_docs(project_root: Path, roles: List[str], force: bool = False) -> Dict[str, Any]:
    ...
    return {
        ...
        "docs_root": rel_to_project(root, project_root),
        "roles_created": roles,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Initialize project-state role documentation for a project.",
    )
