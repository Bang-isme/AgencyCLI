...
def doc_ref(role: str, filename: str, docs_base_rel: str = ".agency/project-docs") -> str:
    return normalize_rel(Path(docs_base_rel) / role / filename)
...
def add_suggestion(
    ...
    candidate = {
        ...
        "role": role,
        "doc": doc_ref(role, str(doc["file"]), docs_base_rel),
        "doc_id": str(doc["id"]),
        "reason": reason,
    }
...
def map_changed_file(path: str, manifest: Dict[str, Any], docs_base_rel: str = ".agency/project-docs") -> List[Dict[str, str]]:
    normalized = normalize_rel(path)
    lowered = normalized.lower()
    suggestions: List[Dict[str, str]] = []

    def add(role: str, doc_id: str, reason: str) -> None:
        add_suggestion(suggestions, normalized, role, find_doc(manifest, role, doc_id), reason, docs_base_rel)

    if lowered.startswith(".codex/project-docs/") or lowered.startswith(".agency/project-docs/"):
        return []
...
def expected_doc_paths(manifest: Dict[str, Any], docs_base_rel: str) -> List[str]:
    paths: List[str] = [
        normalize_rel(Path(docs_base_rel) / "PROJECT-BRIEF.md"),
        normalize_rel(Path(docs_base_rel) / "decisions" / "ADR-0001-template.md"),
    ]
    for role, payload in manifest["roles"].items():
        for doc in payload["docs"]:
            paths.append(doc_ref(role, str(doc["file"]), docs_base_rel))
    return paths
...
def collect_missing_docs(project_root: Path, manifest: Dict[str, Any]) -> List[str]:
    ...
    docs_base_rel = normalize_rel(root.relative_to(project_root))
    for rel_path in expected_doc_paths(manifest, docs_base_rel):
...
def check_role_docs(project_root: Path, changed_files: List[str] | None = None) -> Dict[str, Any]:
    ...
    for changed_file in files:
        suggestions.extend(map_changed_file(changed_file, manifest, docs_base_rel))
