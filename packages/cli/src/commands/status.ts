...
export function buildFlagRows(f: Flags): { label: string; value: string; keys: (keyof Flags)[] }[] {
  return [
    ...
    { label: "Scoped circuit breaker", value: f.scopedCircuitBreaker ? `on (per-turn/agent breaker, threshold≤${f.circuitBreakerThreshold})` : `off (shared module singleton, threshold≤${f.circuitBreakerThreshold})`, keys: ["scopedCircuitBreaker", "circuitBreakerThreshold"] },
