...
## 🔴 P0 — CHURN GHI FILE LỚN VẪN ĐANG XẢY RA (user báo qua ảnh mới, 2026-06-02)
**Bằng chứng (ảnh):** project `D:\AnimeSoul\aniverse`, file `src/data/anime-data.ts`. Trace thật: `npm run build` exit 1 → read anime-data.ts (33 lines) → read types.ts (441) → `append_file` 6.6KB → build exit 1 → `Remove-Item anime-data.ts -Force` → `append_file` 4.6KB → build exit 1 → `Remove-Item` → … **14m56s, 23.8k tokens, status kẹt "Writing".** Model tự nói: *"File vẫn bị cắt – nó lại bắt đầu từ giữa"*, *"File bị cắt mất phần đầu"*, *"Cần xóa và tạo lại toàn bộ"*.
**Chẩn đoán (verify TRƯỚC khi sửa, nhưng đây là giả thuyết mạnh):** triệu chứng "bắt đầu từ giữa / mất phần đầu" = **đúng dấu hiệu bug reassembly `cb932d8`**: `write_file` lớn bị output-limit cắt thiếu `</tool_call>` → `parseToolCalls` drop → phần ĐẦU không bao giờ ghi → model append đuôi lên file thiếu-đầu → "corrupted" → Remove-Item → rewrite → churn. **GỐC RỄ HỆ THỐNG:** các fix churn (`cb932d8` reassembly · `9a0a03f` truncate-tail · `34a0f23`/`4d97563` auto-continue · `1c6fc75` resume-continuation) **ĐỀU flag-gated OFF ở legacy** → **user chạy legacy nên KHÔNG hưởng** → vẫn churn y như chưa fix. (Memory đã cảnh báo pattern này nhiều lần.)
**HÀNH ĐỘNG P0 (theo thứ tự):**
1. **Cứu trợ tức thì cho user:** hướng dẫn chạy `AGENCY_PROFILE=hardened` (bật cả cụm) HOẶC tối thiểu `AGENCY_TOOLCALL_REASSEMBLY=1 AGENCY_TOOLRESULT_TAIL=1 AGENCY_AUTO_CONTINUE=1 AGENCY_RESUME_CONTINUATION=1` + **`pnpm -r build` + restart TUI**. Đây là cách nhanh nhất hết churn. **NHẮC user ngay đầu phiên.**
2. **Reproduce + verify** churn với cụm cờ OFF (tái hiện) rồi ON (hết). Nếu ON vẫn churn → có bug DƯ mà reassembly chưa phủ (vd: chiến lược write-then-append khi write đầu bị drop; hoặc resume-guidance chưa đủ mạnh để model ĐỌC trạng thái đĩa thay vì Remove-Item+rewrite). Điều tra `chat/stream.ts`+`orchestrator.ts` turn-loop với reassembly ON.
3. **ĐỀ XUẤT user promote cụm-churn → ON-by-default (legacy cũng bật).** Ảnh này là bằng chứng đủ mạnh: 4 fix churn vô dụng nếu off-default. **CẦN user OK rõ** (đừng tự promote — §6.1/standing constraint), nhưng hãy CHỦ ĐỘNG đề xuất + nêu rủi ro (eval legacy↔hardened cho cụm này). Đây là giao điểm với BYOK-promote nhưng **không cần key** (churn-cluster là correctness, không phải provider).
...
- **(e) 🔥 SKILLS/PLUGIN PIPELINE END-TO-END AUDIT (mandate mới của user — ưu tiên cao):** verify TIGHT-COUPLING + wired-or-dead + chống-trùng cho TOÀN pipeline: **harness skill-execution ↔ 20 built-in tools (1 ToolRegistry) ↔ skills (định nghĩa/manifest) ↔ prompt+markdown+references+scripts+starter+template (skill assets) ↔ domain-specialist subagents ↔ spawn parallel subagents (`dispatch_subagent`, maxParallelAgents) ↔ persistent memory (markdown §9 + episodic SQLite) xuyên long/multi-session**. Mục tiêu: mỗi mắt xích (a) NỐI DÂY thật (không built-but-unwired), (b) KHÔNG trùng lặp, (c) workflow từ skills CHÍNH chạy hết công năng đúng "ý nghĩa skills-plugin". Cách làm: tra `docs/PACKAGES.md` Canonical Homes; map luồng từ skill manifest → harness → tool/subagent → memory; viết test e2e cho 1-2 skill chính chạy full pipeline; tìm mắt xích chết/lỏng. **Đây là luồng lớn — chia slice, mỗi slice 1 mắt xích, verify từng cái.** (Liên quan: 6 guard skills↔manifest/agents↔prompt/seed đã có — mở rộng nếu cần.)
...
**Bắt đầu:** đọc §0 → **NHẮC user `pnpm -r build` + restart TUI + bật cụm cờ churn (P0 bước 1)** → xử **P0 (churn vẫn xảy ra)** trước: reproduce với cờ OFF, verify ON hết churn, điều tra bug dư nếu còn, đề xuất promote cụm-churn→default (user OK). Sau P0 → **(e) skills-pipeline audit** (mandate mới). Làm theo nhịp §5. **BYOK CUỐI.**
