...
- **§8.8 sandbox/security — 3 lỗi THẬT (2 từ ảnh user)**: `c03b9a2` **self-kill HARD-refuse** (`runShellCommand` cũ in "Blocked command" rồi **chạy tiếp**; `execute_command` hardcode `yes:true` bypass approval → `taskkill /IM node.exe` tự sát TUI → giờ throw trước exec, `yes` không bypass, message chỉ lối an toàn) · `71cbe78` **DockerSandbox timeout+output-cap** (parity native — trước treo vô hạn/OOM) · `ca2c954` **circuit-breaker fire-on-blocked-loop** (`executeTool` recordToolSuccess cho cả chuỗi `Error…` → nhánh consecutiveFailures CHẾT → model churn variant; giờ đếm `^Error[:\s]` là failure + reset breaker mỗi turn + bound history).
- **Baseline verify:** build 16/16, **REAL_EXIT_CODE=0**, **~2134 test** (core **396** · cli 573 · tui **148** · providers 852 · memory 36 · benchmark 18 · workspace 11 · security **38** · tooling 14 · governance 7 · skills-bridge 14 · context 6 · heuristics 6 · browser 5 · telemetry 9). **31 cờ** (thêm `resumeContinuation`, `compactToolDocs` so với 29). **18 tool.**
...
### 🟠 Follow-up trực tiếp từ ảnh user (ưu tiên cao, verify KHÔNG cần key)
- **(A) Hard-break turn loop trên circuit-breaker trip.** `ca2c954` đã làm breaker FIRE (đếm `Error…` result là failure, reset mỗi turn) nhưng trip CHỈ trả message string (soft) — turn loop `chat/stream.ts`/`orchestrator.ts` CHƯA hard-break → model phớt lờ vẫn churn tới maxLoops. **SỬA:** turn loop phát hiện `executeTool` trả "circuit breaker triggered" (hoặc đếm N tool-result lỗi liên tiếp) → break + final assistant message rõ. Cờ nếu đổi hành vi.
- **(B) Malformed `</tool_call>` từ model (minimax).** Ảnh cho thấy model emit nhiều `</tool_call>` lạc/thiếu `>`. Kiểm `parseToolCalls` (`skill/tool-harness.ts`) có nuốt gọn XML hỏng/lồng nhau + không double-execute không; cải thiện robustness nếu cần.
