...
| Turn completion-detection (did a no-tool-call turn signal it's unfinished?) + bounded auto-continue nudge | `core/src/chat/turn-helpers.ts` (`detectIncompleteCompletion`, `buildAutoContinueNudge`, `MAX_AUTO_CONTINUE`) | Conservative end-anchored detector (first-person "I'll continue" promise / "to be continued" / `// ...rest of the code` placeholder; excludes user-offers + questions). Wired into the no-tool-call `else` branch of BOTH turn loops (`stream.ts`+`orchestrator.ts`), gated by `autoContinue` — off → byte-identical `break`. `buildAutoContinueNudge` shares the `describeFilesWritten` listing helper with `buildIncompleteTurnNotice`. Don't add a second completion-heuristic; extend the detector here. |
