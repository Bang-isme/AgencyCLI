...
| Route → display strings (`formatRouteSummary`, `buildSuggestedCommands`) | `core/src/chat/route-presentation.ts` | Pure helpers (only a type-import of `RouteResult` + leaf `getInvokeActions`). Lives in its own leaf so `context/pack.ts` and `agents/orchestrator.ts` don't import `chat/orchestrator.ts` — that back-edge formed a runtime import cycle (`orchestrator → turn-helpers → context/pack → orchestrator`, and via `prompt → tool-harness → agents/orchestrator → orchestrator`). `orchestrator.ts` re-exports both, so existing `from "./orchestrator.js"` consumers keep one import path. |
