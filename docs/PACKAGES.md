...
| `markdown-memory.ts` | `MarkdownMemoryStore` — curated cross-session memory as human-readable markdown (`.agency/memory/` index + topic files w/ frontmatter); deliberate `remember`-tool writes + recall (keyword or `Embedder` semantic), distinct from the auto episodic store. See Canonical Homes. |
