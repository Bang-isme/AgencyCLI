...
| `markdown-memory.ts` | `MarkdownMemoryStore` — curated cross-session memory as human-readable markdown (`.agency/memory/` index + topic files w/ frontmatter); deliberate `remember`/`forget`-tool writes + budgeted keyword recall, distinct from the auto episodic store. See Canonical Homes. |
...
|---------|----------------|-------|
| Chat-turn setup (route resolve, key check, context repack), context compaction, turn token-cost estimate | `core/src/chat/turn-helpers.ts` | Shared by BOTH `runChatTurn` (orchestrator.ts) and `runChatTurnWithStream` (stream.ts). `recordTurnTokenCost` consolidated the once-copy-pasted `~len/4 + 200` estimate. `compactTurnHistory` is invoked via a single `compactIfEnabled` closure per turn path — **before the loop AND at the top of each outer-loop iteration** (so tool results accumulating mid-turn are compacted too), gated by `contextCompaction`, incremental across calls via the `cacheKey`. |
| Turn completion-detection (did a no-tool-call turn signal it's unfinished?) + bounded auto-continue nudge | `core/src/chat/turn-helpers.ts` (`detectIncompleteCompletion`, `detectTruncatedArtifact`, `buildAutoContinueNudge`, `MAX_AUTO_CONTINUE`) | Two complementary signals: (a) `detectIncompleteCompletion` — conservative end-anchored detector on the model's OUTPUT text (first-person "I'll continue" promise / "to be continued" / `// ...rest of the code` placeholder; excludes user-offers + questions); (b) `detectTruncatedArtifact` — scans the ON-DISK contents of files the turn wrote for the same `CODE_PLACEHOLDER` elision marker (catches a saved stub even when a later message looks clean). Both feed the no-tool-call `else` branch of BOTH turn loops (`stream.ts`+`orchestrator.ts`), gated by `autoContinue` — off → byte-identical `break`. `buildAutoContinueNudge` shares the `describeFilesWritten` listing helper with `buildIncompleteTurnNotice`. Don't add a second completion-heuristic; extend the detectors here. |
| Route → display strings (`formatRouteSummary`, `buildSuggestedCommands`) | `core/src/chat/route-presentation.ts` | Pure helpers (only a type-import of `RouteResult` + leaf `getInvokeActions`). Lives in its own leaf so `context/pack.ts`, `agents/orchestrator.ts` and `chat/presentation.ts` don't import `chat/orchestrator.ts` — that back-edge formed a runtime import cycle (`orchestrator → turn-helpers → context/pack → orchestrator`, and via `prompt → tool-harness → agents/orchestrator → orchestrator`). `orchestrator.ts` re-exports both, so existing `from "./orchestrator.js"` consumers keep one import path. Guarded by `core/__tests__/architecture-cycles.test.ts` (below). |
...
| First-class runtime state (plan/steps, modified files, tool + agent health, continuations, warnings, cost) derived from events | `core/src/runtime/runtime-state.ts` — `reduceRuntimeState(events)` (PURE fold) + `loadRuntimeState(projectRoot)` (folds the durable journal) | The single journal→state derivation (AGENT_OS K1 / EVENT_FIRST §3). `agency status` consumes it (flag `runtimeState`); the future Activity Timeline / Tasks panels **and** the supervisor MUST fold this same reducer — do NOT add a second state derivation. No new write path — it only folds events the bus already emits. |
...
| Curated cross-session markdown memory (index + topic files; `remember`/`forget` tools) | `@agency/memory` `MarkdownMemoryStore` — recall wired via `core/src/chat/memory-integration.ts` `loadFileMemoryBlock` (prepended to the `### SYSTEM HISTORICAL MEMORIES` block); written via `remember` / removed via `forget` built-in tools in `skill/tool-harness.ts` | A small set of DELIBERATELY-saved, human-readable facts (user prefs, decisions, "don't re-investigate" findings) under `.agency/memory/` (`MEMORY.md` index + `<slug>.md` topic files with frontmatter). **Distinct from the automatic SQLite episodic store** (`EpisodicStore`/`safeAddEpisode`, which auto-logs every tool call/reply) and from the `handover.ts` resume snapshot. Gated by `fileMemory` (on by default in both profiles; opt out with `AGENCY_FILE_MEMORY=0`); recall orders `project`/`reference` by keyword overlap (small curated set — kept deliberately simple, no embedder), `user`/`feedback` always surfaced. Don't add a second markdown-memory store or a parallel recall path. |
| Model catalog + cost resolution | `@agency/providers` (`model-catalog.ts`) + governance cost resolver | One price table; `matchModelKey` is shared with `resolveModelSpec`. `getCatalogSpec(model, providerId?)` is provider-aware (§8.2): with a providerId it clamps the context window to the conservative cross-provider robust-min (drops <50%-of-max router outliers) so a wrong-high catalog/registry entry can't over-allocate — `getModelSpec`/`getTokenBudgetPlan` thread providerId through. |
...
| Runtime import-cycle invariant (no layering violations) | `core/__tests__/architecture-cycles.test.ts` (Tarjan SCC over `core/src` value-imports) | Asserts the ONLY multi-module runtime cycle is the irreducible *functional* one — `{agents/orchestrator, chat/orchestrator, chat/prompt, chat/stream, chat/turn-helpers, skill/tool-harness, task/runner}` (turn↔tools↔dispatch↔turn + setup + gate). Any module OUTSIDE that set landing in a cycle fails the test — that's the recurring defect (a lower/utility/presentation layer importing `chat/orchestrator` for a helper; fix by moving the helper to a leaf, not by widening the set). |
| Package-level import-cycle invariant (no `@agency/* ↔ @agency/*` cycle) | `cli/src/__tests__/package-cycles.test.ts` (Tarjan SCC over `from "@agency/<pkg>"` value-imports across all `packages/*/src`) | Asserts **zero** runtime cycles between workspace packages. The `core ↔ skills-bridge` cycle existed until the bridge's `runTool` approval policy was injected from the CLI instead of imported from core. Fix a failure by inverting the back-edge (push the shared concern to a leaf package, or inject it from the caller). |
| Workspace dependency hygiene (declared `@agency/*` deps == imported) | `cli/src/__tests__/package-cycles.test.ts` (the `package dependency hygiene` describe — shares the file/scaffolding with the cycle guard) | Asserts every `@agency/*` in a package's `dependencies` is actually imported in its `src` — no **dead** dep (the package-level analogue of a dead export; caught cli's leftover `@agency/telemetry`, whose trace type tsc resolves transitively through `@agency/benchmark`'s own `node_modules`, so cli never needed a direct dep) — and every imported `@agency/*` is declared — no **phantom** dep that resolves only via pnpm hoisting and breaks on strict install. Captures **all** import forms incl. dynamic / inline `import("@agency/x")` (e.g. `await import("@agency/tui")`). |
| Plugin-tool approval gate (policy for `skills-bridge` `runTool`) | `cli/src/plugin-approval-gate.ts` `pluginApprovalGate` | The bridge is pure mechanism (run the Python script) and takes the policy via its `onBeforeRun` hook; the CLI owns the gate (warn + `system:warning` + audit when a write-capable tool runs without approval) so `@agency/skills-bridge` no longer imports `@agency/core`. Don't move this policy back into the bridge. |
