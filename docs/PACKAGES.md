...
| Runtime cognition narration (`thought:emitted` for the TUI CognitionPanel) | `core/src/events/cognition.ts` `emitThought` | Single producer; the CognitionPanel (`tui/App.tsx`) is the single consumer. Gated centrally by `cognitionStream`. Don't publish `thought:emitted` ad-hoc — call `emitThought`. |
