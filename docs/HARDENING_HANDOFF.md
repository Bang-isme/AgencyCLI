...
## 3. What is DONE (verified 2026-06-01 via `pnpm verify` — self-run, not inherited: full `pnpm -r build` clean 16/16; core 328 / cli 556 / tui 115 / memory 34 / workspace 11 / benchmark 14 / governance 7 / providers 840 / security 35 / tooling 14 / skills-bridge 13 / context 6 / heuristics 6 / browser 5 / telemetry 4 — **~1988 tests pass, exit 0**, full `pnpm -r` sweep green under concurrent load. Core moved 350→…→348 (§2.5 trace-recorder); cli 550→556 (§2.5 `replay-regression` driver, +6); core 348→328 (deleted 2 dead modules found by extending the wired-or-dead audit beyond `core/index.ts` — see §5 cont'd 9). Wired-or-dead initiative closed 100% for index.ts exports, then extended to non-exported modules.)
...
**25 flags total** (added `traceRecord`/`AGENCY_TRACE_RECORD`, §2.5). All resolve via `getRuntimeFlags()` (env override → `AGENCY_PROFILE` default → built-in).
