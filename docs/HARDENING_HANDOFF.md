...
>
> **NEXT:** for a clean success-*rate* delta, add a task this model fails attempt-1 *reliably* (merge-intervals is non-deterministic at the boundary) or run vs a weaker model · full §2.5 = agent re-execution (needs recording LLM responses too) · optional §2.4 typed/structured tool results (marginal) · then promote `hardened`→default.
