...
>
> **§2.3 in-loop compaction + a route-presentation cycle untangle (2026-06-01, cont'd 21).** Closed the §2.3 "in-loop compaction" follow-up that ROADMAP §2.3 had explicitly deferred. Both turn paths compacted the history **once, before** the outer tool-loop; but `turnHistory` keeps growing *inside* the loop (each iteration appends the assistant turn + tool results), and the reactive context-limit handler only shrinks the *window*, never the *conversation* — so a long tool-loop could still overflow mid-turn. Hoisted the inline `if (contextCompaction) {…}` into a single `compactIfEnabled()` closure (per turn path) and call it **before the loop AND at the top of every iteration**; reuses the canonical `compactTurnHistory` (no new logic), the existing `cacheKey` makes the repeated in-loop compactions incremental (O(new turns)), and it's a no-op under threshold + byte-identical when `contextCompaction` is off. **No new flag.**
> - **The test exposed — and I removed — a real import cycle.** The intended unit test spies on `compactTurnHistory` (counting pre-loop + per-iteration = 3), but the spy never registered: `chat/orchestrator.ts ↔ chat/turn-helpers.ts` sit on a runtime import cycle, so a partial mock whose factory called `importOriginal()` pulled the real orchestrator back in and bound it to the *real* helper. The cycle ran through two **layering-violation** back-edges — `context/pack.ts` and `agents/orchestrator.ts` importing `chat/orchestrator.ts` just for the pure helpers `formatRouteSummary` / `buildSuggestedCommands`. Relocated both into a new leaf module **`chat/route-presentation.ts`** (only a type-import of `RouteResult` + leaf `getInvokeActions`); `orchestrator.ts` re-exports them so every `from "./orchestrator.js"` consumer (index.ts, stream.ts) is unchanged. This is a behaviour-preserving relocation (the helpers were copied verbatim, `Set`-dedup and all). The remaining edge (`agents/orchestrator → stream → orchestrator`, i.e. dispatch runs a chat turn) is a *legitimate* functional cycle and was left intact; the test instead uses cycle-immune stub helpers (no `importOriginal`), so the orchestrator binds the spy.
>
> `pnpm verify` green (build 16/16, exit 0): core 344→**347** (+3 in-loop-compaction tests now pass), repo ~2030→**~2033**, **27 flags** (no new flag).
>
> **Architecture cycle audit + guard (2026-06-01, cont'd 22).** Followed up cont'd 21 with a systematic Tarjan-SCC pass over the whole `core/src` runtime import graph. Result: a single 8-module cycle cluster, of which **one edge was still a leftover layering violation** — `chat/presentation.ts` importing `buildSuggestedCommands` from `chat/orchestrator.ts` (the *third* consumer of the relocated route-presentation helpers; I'd fixed `context/pack.ts` and `agents/orchestrator.ts` in cont'd 21 but missed this one). Repointed it to the `chat/route-presentation.ts` leaf — a one-line, behaviour-preserving swap that drops `chat/presentation` out of the cycle (SCC **8→7**). The remaining 7-module SCC — `{agents/orchestrator, chat/orchestrator, chat/prompt, chat/stream, chat/turn-helpers, skill/tool-harness, task/runner}` — is the **irreducible functional cycle** (a chat turn runs tools → a tool dispatches a sub-agent → the sub-agent runs a chat turn, + shared setup `turn-helpers→prompt→tool-harness` + gate `task/runner→agents/orchestrator`); breaking it would need DI/lazy imports and is intentionally left intact. **Locked it in with a 4th wiring/architecture guard:** `core/__tests__/architecture-cycles.test.ts` computes the SCCs and fails if any module *outside* that known-functional set lands in a runtime cycle — so the tangle can shrink but never silently grow (catches the exact recurring "lower layer reaches up into the orchestrator for a helper" defect). Negative-tested: re-introducing the `presentation→orchestrator` edge makes the guard fail naming the offender. `pnpm verify` green (build 16/16, exit 0): core 347→**348** (+1 guard), repo ~2033→**~2034**, **27 flags** (no new flag). The four wiring guards are now: skills↔manifest, agents↔prompt/seed, flags↔status, **architecture↔cycles**.
>
> **Broke the `core ↔ skills-bridge` PACKAGE cycle via approval-gate dependency-inversion (2026-06-01, cont'd 23).** Extended the cycle audit from module-level (cont'd 22) to PACKAGE-level (Tarjan SCC over `from "@agency/<pkg>"` edges): exactly one runtime package cycle existed — `@agency/core ↔ @agency/skills-bridge` (a known smell the campaign had worked *around*, e.g. parking the skills-manifest guard in cli because core couldn't import skills-bridge in a test). The whole cycle rested on `skills-bridge/src/runner.ts` reaching into core for its `runTool` approval policy: `requiresApproval` + `appendAudit` (`@agency/core/approval`) + `EventBus` (`@agency/core`). That policy is an *orchestration* concern wrongly embedded in a *mechanism* lib — and it was effectively inert anyway (both production callers, `doctor`/`plugin`, pass `yes:true` with no `projectRoot`, so it never warned/audited). **Dependency-inverted it:** `runTool` now takes an optional `onBeforeRun(ctx)` hook (pure mechanism — run the script; absent ⇒ no gate), the policy moved verbatim into `cli/src/plugin-approval-gate.ts` `pluginApprovalGate`, and `doctor`/`plugin` inject it. `@agency/skills-bridge` no longer imports `@agency/core` at all — removed it from the package's `dependencies` too. Behaviour-preserving (the gate is byte-identical, still wired, still fires for a non-`yes` caller; the security capability was relocated, not removed). **Locked in with the 5th guard:** `cli/__tests__/package-cycles.test.ts` asserts ZERO runtime cycles between `@agency/*` packages (negative-tested: a probe re-adding the core import fails it naming `core ↔ skills-bridge`). Bridge test updated to inject a spy hook and assert the contract. `pnpm verify` green (build 16/16, exit 0): cli 571→**572**, skills-bridge 13→**14** passed, repo ~2034→**~2036**, **27 flags** (no new flag). Five guards now: skills↔manifest, agents↔prompt/seed, flags↔status, architecture↔cycles (module), **package↔cycles**.
>
> **Symbol-level dead-export prune — the finest layer of the wired-or-dead thesis (2026-06-01, cont'd 24).** With the dup-scan clean and the module-level wired-or-dead closed, swept the next-finer granularity: an exported symbol in a deep (non-index) module that is referenced *nowhere* — the same "built-but-unwired" defect one level down. A precise sweep over all 820 non-index/non-test exports (importers checked across `.ts`/`.tsx`/`.mts`, **including tests**, so a test-only fixture isn't flagged) separated **truly-dead** (zero references) from merely **over-exported** (used only inside its own file — live, e.g. `LeaseManager`/`globalLeaseManager`, the `tasks.ts` corpus consts) and found **13 truly-dead exports**. Removed all 13 (behaviour-preserving — each had zero consumers incl. tests, and `noUnusedLocals` confirms no orphaned imports; the live siblings `FailureNormalizer`/`touchProject`/`globalWorkerTracker`/`circuitBreakerState`/the other `agent-modes` helpers were kept): `FailureClassifier`+`FailureCategory` (`task/runner.ts` — a speculative stderr classifier superseded by the live `FailureNormalizer.normalize`/`hash`), `getInstallCommand` (`utils/package-manager.ts` — install isn't an acceptance step), `resetCircuitBreaker` (`skill/tool-harness.ts` — an uncalled reset helper; the `circuitBreakerState` it touched is still live at the tool-loop check/record sites), `addProject`+`removeProject` (`tui/sessions/projects.ts` — superseded by `touchProject`'s upsert; no remove-from-recents UI), `modeSkillPrefix` (`tui/state/agent-modes.ts` — a dead mode→skill-prefix helper in an otherwise-live module), `YOGA_SAFE_MARGIN` (`tui/layout/terminal-layout.ts`), `hasPendingStdinBytes` (`tui/terminal/screen.ts`), and `SemanticEvent` + `semanticEventBus` (its sole-consumer `EventBus` import removed too) + the orphan `TemporalChoreographyEngine`/`globalChoreography` liveness-heartbeat (`tui/state/semantic-orchestration.ts` — the heartbeat never started since nothing referenced the singleton; the real TUI elapsed/heartbeat timers superseded it). **A first quick sweep was truncated at the alphabetical `h…` boundary and missed `modeSkillPrefix`/`resetCircuitBreaker`/`semanticEventBus` — re-running it to completion before commit caught all three (verify-don't-assert).**
>
> **Workspace dependency-hygiene audit + 6th guard — a dead workspace dep removed (2026-06-01, cont'd 25).** Continued the cycle/architecture sweep to package-manifest hygiene: per package, declared `@agency/*` `dependencies` vs `@agency/*` actually imported in `src`. The graph was clean **except `@agency/cli`**, which declared `@agency/telemetry` with no import — only a comment. The cont'd-8 note assumed cli needed it "so tsc resolves the trace type" (`runRegressionReplay(trace: DeterministicExecutionTrace, …)` leaks a telemetry type through `@agency/benchmark`'s signature). **Empirically falsified** (verify-don't-assert, per cont'd 24): removed the dep → `pnpm install` → clean-rebuilt cli from scratch → tsc resolves `DeterministicExecutionTrace` **transitively through `@agency/benchmark`'s own `node_modules`** (where the `.d.ts` importing it lives), not through cli's. So `@agency/telemetry` was **dead weight** — the package-level analogue of a dead export / the cont'd-23 `skills-bridge` cleanup — removed from `package.json` + `pnpm-lock.yaml` (3-line scoped diff). cli has no runtime telemetry import and the `replay-regression` tests pass without it; also fixed the now-doubly-misleading comment (its cited `replay`/`@agency/contracts` precedent was bogus — `replay.ts` never references contracts). **Locked in with the 6th guard:** a `package dependency hygiene` describe in `package-cycles.test.ts` (reuses its `packageDirs`/`listSourceFiles` scaffolding — no new file/duplication) asserts declared `@agency/*` deps == imported per package (catches **dead** AND **phantom** deps; captures all import forms incl. dynamic `import("@agency/x")` so cli's `await import("@agency/tui")` isn't false-flagged). Negative-tested: re-adding the dead dep fails it naming `@agency/cli: declares "@agency/telemetry" but never imports it`. `pnpm verify` green (build 16/16, exit 0): cli 572→**573** (+1 guard), repo ~2036→**~2037**, **27 flags** (no new flag). **6 guards: skills↔manifest, agents↔prompt/seed, flags↔status, architecture↔cycles (module), package↔cycles, deps↔imports hygiene.**
>
> **Whole-file orphan sweep (all packages) — 2 unwired benchmark scripts removed (2026-06-01, cont'd 26).** Swept the dimension *between* the core-only module sweep (cont'd 9) and the symbol sweep (cont'd 24): a file whose exports are used only *within itself* (so the symbol sweep marks them live) yet which **nothing imports**. Collected every imported module-specifier basename in one pass, then flagged any non-entry `src` file whose basename appears in none — across ALL packages. Clean except `vitest.config.ts` (×5, legit vitest entry points) and **`packages/memory/src/__benchmarks__/{benchmark-memory,chaos-stress}.ts`**. Both were **built-but-unwired**: self-executing (`runBenchmark()` / `runChaosStressBenchmark().catch(...)` at module level) but run by nothing (no package.json script, no import, no doc, no CI), compiled into the **shipped** `dist` (`files: ["dist"]` → dead weight in the npm tarball), and `chaos-stress.ts` **duplicated** the live `chaos-stress.test.ts` (8 tests covering the same 1000-writes / ENFILE-ENOSPC / WAL-corruption / cost-ceiling scenarios). **Deleted both** (cont'd-9 precedent; recoverable from git; if perf benchmarking is wanted later it should be re-added *wired* — excluded from the production build + a `bench` script). Zero importers confirmed across the whole repo (.ts/.tsx/.mts/.cts incl tests). **The cont'd-25 dependency-hygiene guard paid off on the very next slice:** deleting `chaos-stress.ts` removed memory's *only* import of `@agency/governance` (the benchmark's `import { CostGovernor, CostGovernanceCeiling }`), so the first `pnpm verify` after the deletion *failed* with the guard naming the now-dead dep — `@agency/memory: declares "@agency/governance" but never imports it`. Removed it from `packages/memory/package.json` + `pnpm-lock.yaml` too (3-line scoped diff; confirmed zero memory src/test imports of governance). Documented the orphan sweep as a repeatable command in PACKAGES.md (lint-style, not a CI guard — entry points/config files need allowlisting, like the dead-export sweep). `pnpm verify` green (build 16/16, exit 0): counts unchanged (the deleted files were never tested/imported), **27 flags**. The semantic-orchestration file stays LIVE (App.tsx consumes `globalWorkerTracker`/`SemanticTranslator`/`WorkerState`) — only its orphan exports were trimmed, exactly the finer cleanup the false-dead lesson permits. **Documented as a repeatable sweep (not a CI guard)** in PACKAGES.md next to the dup-scan — dead-export-zero is a lint-style concern (a util committed just before its consumer false-positives), better served by `knip`/`ts-prune` than a bespoke brittle `pnpm verify` test. `pnpm verify` green (build 16/16, exit 0): counts unchanged (no tests touched — cli **572**, repo **~2036**), **27 flags**. The symbol-level sweep is now **empty**.
