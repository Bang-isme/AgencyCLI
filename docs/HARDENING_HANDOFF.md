...
>
> **NEXT:** promote `hardened`→default once a clean eval delta is in hand · **full §2.5 live re-execution** — run the REAL `runChatTurn` against a `ReplayProvider` (an `LlmProvider` returning the recorded `llmResponses` in order) + a tool-intercept seam, so a recorded session re-runs with zero LLM calls and reuses the real turn loop (no logic duplication); the LLM-response data is the prerequisite, now in place · for a clean success-*rate* delta, add a task this model fails attempt-1 *reliably* or run vs a weaker model · cross-turn running-summary compaction (needs session-history threading) · optional §2.4 typed/structured tool results (marginal).
