...
>
> **NEXT:** **full §2.5 live re-execution** — run the REAL `runChatTurn` against a `ReplayProvider` (an `LlmProvider` returning the recorded `llmResponses` in order) + a tool-intercept seam, so a recorded session re-runs with zero LLM calls and reuses the real turn loop (no logic duplication); the LLM-response data this slice records is the prerequisite, now in place · for a clean success-*rate* delta, add a task this model fails attempt-1 *reliably* (merge-intervals is non-deterministic at the boundary) or run vs a weaker model · optional §2.4 typed/structured tool results (marginal) · then promote `hardened`→default.
