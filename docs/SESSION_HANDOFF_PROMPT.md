...
- Gate DUY NHẤT: `pnpm verify` (= `pnpm -r build && pnpm -r test`). Baseline: build 16/16 sạch, **~2026 test, exit 0** (core 343 / cli 568 / tui 115 / providers 840 / memory 36 / benchmark 18 / workspace 11 / security 35 / tooling 14 / governance 7 / skills-bridge 13 / context 6 / heuristics 6 / browser 5 / telemetry 9).
...
- **NEXT không cần key:** (a) full §2.5 = re-execute agent THẬT — chạy lại `runChatTurn` qua một **ReplayProvider** (`LlmProvider` trả `llmResponses` đã ghi theo thứ tự) + seam inject provider/intercept tool, để tái dùng vòng turn THẬT (không nhân đôi loop); dữ liệu LLM-response giờ đã có sẵn cho việc này; (b) **running-summary xuyên lượt** cho compaction (chỉ tóm phần MỚI → khử O(n) cost; cần luồn history qua session store); (c) §2.4 typed/structured tool-result (giá trị biên); (d) thêm bài eval khó-hơn-ổn-định (rate-delta legacy<hardened sạch).
