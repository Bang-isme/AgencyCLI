...
- **4 tầng wiring/kiến trúc đã có REGRESSION GUARD (chạy trong `pnpm verify`) — đừng để drift:** skills↔manifest (`cli/__tests__/skills-manifest-integrity.test.ts`: mọi skill khai có SKILL.md + ngược lại + `manifest.agents`==`MANIFEST_AGENTS` + load_order); agents↔prompt/seed (`core/__tests__/agent-dispatch-integrity.test.ts`: `AGENT_SUBAGENT_PROMPT`/seed/`MANIFEST_AGENTS` đồng bộ + prompt file resolve + disciplines là skill thật); flags↔status (`cli/__tests__/status-flags.test.ts`: mọi flag `getRuntimeFlags()` đều xuất hiện ở `agency status`); **architecture↔cycles (`core/__tests__/architecture-cycles.test.ts`: Tarjan-SCC trên `core/src`, fail nếu MODULE NGOÀI cycle chức năng 7-module known lọt vào một runtime import cycle — bắt đúng lỗi "tầng dưới import lên `chat/orchestrator` để lấy helper"; fix bằng dời helper sang leaf, KHÔNG nới set).** **Thêm skill/agent/flag mới → cập nhật manifest/seed/buildFlagRows trong CÙNG slice** kẻo guard fail. Runtime: `agency doctor` cũng validate pack ĐÃ CÀI (mọi skill khai resolve SKILL.md).
